...
class TicketStatusHistoryResource extends JsonResource
{
    ...
    {
        $internalAssignment = ($request->user()?->hasRole('requester') ?? false) && ($this->action === 'assigned' || str_starts_with($this->action, 'analysis_') || str_starts_with($this->action, 'solution_plan_') || str_starts_with($this->action, 'development_') || str_starts_with($this->action, 'worklog_') || str_starts_with($this->action, 'internal_test_'));
