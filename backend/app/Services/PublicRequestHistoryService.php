...
final class PublicRequestHistoryService
{
    ...
    /** @return array{challenge_token: string, expires_at: string, resend_available_at: string} */
    public function requestCode(string $email, int $branchId): array
    {
        ...
        $challenge = DB::transaction(function () use ($branchId, $identityHash, $email, $tokenHash, $code, $expiryMinutes, $cooldownSeconds) {
        ...
        if (! hash_equals($challenge->challenge_token_hash, $tokenHash)) {
            return [
                'challenge_token' => $this->randomToken(),
