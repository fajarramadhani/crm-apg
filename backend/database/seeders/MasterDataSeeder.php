...
class MasterDataSeeder extends Seeder
{
    public function run(): void
    {
        if (! app()->environment('testing')) {
            throw new \RuntimeException('MasterDataSeeder is restricted to automated tests. Configure organization data through the Admin API.');
        }
        ...
        foreach ([
            ...
            Division::updateOrCreate(['code' => $code], ['name' => $name, 'description' => 'Automated test fixture', 'is_active' => true]);
        }

        foreach ([['JKT', 'Jakarta', 'Jakarta'], ['SBY', 'Surabaya', 'Surabaya'], ['BDG', 'Bandung', 'Bandung']] as [$code,$name,$city]) {
            Branch::updateOrCreate(['code' => $code], ['name' => $name, 'city' => $city, 'address' => 'Automated test fixture', 'is_active' => true]);
        }
