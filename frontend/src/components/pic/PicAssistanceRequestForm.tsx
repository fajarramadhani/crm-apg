...
export const PicAssistanceRequestForm: React.FC<PicAssistanceRequestFormProps> = ({ onSubmit, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Gagal mengirim permintaan bantuan.')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full p-3 text-sm border border-slate-300 rounded-lg bg-slate-50 text-slate-900 focus:ring-2 focus:ring-blue-500 focus:outline-none"
          required
        />
      </div>

      <div className="p-3 bg-blue-50 border border-blue-200 dark:border-blue-800 rounded-lg text-xs text-blue-800">
