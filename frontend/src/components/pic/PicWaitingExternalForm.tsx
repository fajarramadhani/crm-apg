...
export const PicWaitingExternalForm: React.FC<PicWaitingExternalFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Gagal menandai pihak eksternal.')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          className="w-full p-2.5 text-sm border border-slate-300 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-purple-500 focus:outline-none"
          required
        />
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full p-3 text-sm border border-slate-300 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-purple-500 focus:outline-none"
          required
        />
      </div>
