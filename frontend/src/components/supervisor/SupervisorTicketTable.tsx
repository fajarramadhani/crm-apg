...
export const SupervisorTicketTable: React.FC<SupervisorTicketTableProps> = ({ tickets, loading = false }) => {
  if (loading) {
    return (
          ...
          {tickets.map((t) => {
            ...
            return (
              <tr key={t.id} className="hover:bg-blue-50/30 transition-colors">
                <td className="whitespace-nowrap px-4 py-3.5 font-medium text-blue-900">{t.ticket_number}</td>
                ...
                <td className="whitespace-nowrap px-4 py-3.5">
                  <span
                    className={`inline-flex rounded-md px-2 py-0.5 text-xs font-semibold ${getPriorityBadgeClass(t.final_priority?.key)}`}
                  >
                ...
                <td className="whitespace-nowrap px-4 py-3.5">
                  <span
                    className={`inline-flex rounded-full border px-2.5 py-0.5 text-xs font-semibold capitalize ${getStatusBadgeClass(t.status)}`}
                  >
