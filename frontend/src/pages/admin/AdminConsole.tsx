  ...
  const modules = [
    {
      ...
      icon: '👥',
      path: '/admin/users',
      desc: `${USERS.length} user aktif`,
      color: 'bg-blue-50 border-blue-200',
    },
    {
      label: 'Divisi & Aplikasi',
      icon: '🏢',
      path: '/admin/divisions',
      desc: '9 aplikasi terdaftar',
    ...
    },
    {
      ...
      desc: '4 aturan SLA aktif',
      color: 'bg-amber-50 border-amber-200',
    },
    {
      label: 'Matrix Eskalasi',
      icon: '🔺',
      path: '/admin/escalation',
      desc: '3 level eskalasi',
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            ...
            <div key={s.label} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
              <div
                className={`w-2.5 h-2.5 rounded-full shrink-0 ${s.status === 'online' ? 'bg-emerald-500' : s.status === 'degraded' ? 'bg-amber-500 animate-pulse' : 'bg-red-500'}`}
              />
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-gray-800 truncate">{s.label}</p>
                <p className={`text-xs ${s.status === 'online' ? 'text-emerald-600' : 'text-amber-600'}`}>
                  {s.status} · {s.latency}
                </p>
              </div>
            </div>
