    ...
    <div>
      ...
      {tab === 'Divisi' && (
        <SectionCard
          ...
          actions={
            <Button size="sm" variant="primary" onClick={() => setShowAddDiv(true)}>
              ➕ Tambah Divisi
            </Button>
          }
        >
            ...
            {DIVISIONS.map((div) => (
              ...
              >
                ...
                <div className="hidden group-hover:flex gap-1">
                  <button className="p-1.5 text-xs text-gray-500 hover:text-[#1E3A8A] hover:bg-blue-50 rounded">
                    ✏️
                  </button>
      ...
      {tab === 'Aplikasi' && (
        <SectionCard
          ...
          actions={
            <Button size="sm" variant="primary" onClick={() => setShowAddApp(true)}>
              ➕ Tambah Aplikasi
            </Button>
          }
        >
            ...
            {APPLICATIONS.map((app) => (
              ...
              >
                ...
                <div className="hidden group-hover:flex gap-2">
                  <button className="px-3 py-1 text-xs text-gray-600 border border-gray-200 rounded-lg hover:bg-white">
                    Edit
                  </button>
      ...
      <Modal open={showAddDiv} onClose={() => setShowAddDiv(false)} title="Tambah Divisi">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            <Button variant="secondary" onClick={() => setShowAddDiv(false)}>
              Batal
            </Button>
            <Button variant="primary" onClick={handleSaveDiv} disabled={!newDiv}>
              Simpan
            </Button>
          </div>
        </div>
      ...
      <Modal open={showAddApp} onClose={() => setShowAddApp(false)} title="Tambah Aplikasi">
        <div className="space-y-4">
          <Input
            ...
            placeholder="Contoh: Sistem Manajemen Klaim"
          />
          <Input
            label="Kode Singkat *"
            value={newApp.code}
            onChange={(e) => setNewApp((a) => ({ ...a, code: e.target.value }))}
            placeholder="Contoh: SMK"
          />
          <Input
            label="Owner / PIC Aplikasi"
            value={newApp.owner}
            onChange={(e) => setNewApp((a) => ({ ...a, owner: e.target.value }))}
            placeholder="Tim yang bertanggung jawab"
          />
          <Input
            label="Deskripsi"
          ...
          />
          <div className="flex justify-end gap-2">
            <Button variant="secondary" onClick={() => setShowAddApp(false)}>
              Batal
            </Button>
            <Button variant="primary" onClick={handleSaveApp} disabled={!newApp.name || !newApp.code}>
              Simpan
            </Button>
          </div>
        </div>
