  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        actions={<Button variant="primary" onClick={() => setShowAdd(true)}>➕ Tambah User</Button>}
      />
        ...
        <Table headers={['User', 'Email', 'Role', 'Divisi', 'Status', 'Aksi']}>
          {filtered.map(u => (
            <TR key={u.id}>
              ...
              <TD><span className="text-xs text-gray-600">{u.division}</span></TD>
              <TD>
              ...
              <TD>
                <div className="flex gap-1">
                  <button className="px-2.5 py-1 text-xs text-[#1E3A8A] border border-[#1E3A8A]/30 rounded-lg hover:bg-blue-50 transition-colors">Edit</button>
                  <button className="px-2.5 py-1 text-xs text-red-600 border border-red-200 rounded-lg hover:bg-red-50 transition-colors">Nonaktifkan</button>
                </div>
              </TD>
            </TR>
          ))}
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Tambah User Baru">
        <div className="space-y-4">
          <Input label="Nama Lengkap *" value={form.name} onChange={e => update('name', e.target.value)} placeholder="Nama lengkap user" />
          <Input label="Email *" type="email" value={form.email} onChange={e => update('email', e.target.value)} placeholder="nama@apg.co.id" />
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button variant="primary" onClick={handleSave} disabled={!form.name || !form.email}>Simpan User</Button>
          </div>
        </div>
      </Modal>
    </div>
