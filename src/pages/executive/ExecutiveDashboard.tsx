...
import { KPICard, SectionCard, PageHeader, Button, StatusBadge } from '../../components/ui'
import {
  BarChart,
  Bar,
  XAxis,
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Button variant="primary" onClick={() => navigate('/executive/statistics')}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KPICard
          title="Total Tiket Bulan Ini"
          value={KPI_DATA.totalTickets}
        ...
        <KPICard
          ...
          value={`${KPI_DATA.slaComplianceRate}%`}
          subtitle="Target: 90%"
          color={KPI_DATA.slaComplianceRate >= 90 ? 'green' : 'amber'}
          icon="📊"
        />
        <KPICard
          title="Avg. Resolusi"
          value={`${KPI_DATA.avgResolutionHours}j`}
          subtitle="Target: < 30 jam"
          color={KPI_DATA.avgResolutionHours <= 30 ? 'green' : 'amber'}
          icon="⏱️"
        />
        ...
        <div className="lg:col-span-2">
          <SectionCard title="Tren Tiket 6 Bulan Terakhir">
            ...
            <div className="flex gap-4 justify-center mt-2">
              {[
                { color: '#1E3A8A', label: 'Masuk' },
                { color: '#10B981', label: 'Selesai' },
                { color: '#EF4444', label: 'Over SLA' },
              ].map((l) => (
        ...
        <SectionCard title="Performa Divisi IT">
              ...
              <div key={d.division}>
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-sm font-bold ${d.slaRate >= 90 ? 'text-emerald-600' : d.slaRate >= 75 ? 'text-amber-600' : 'text-red-600'}`}
                    >
                      {d.slaRate}%
                    </span>
                  </div>
          ...
          <div className="mt-4 flex gap-3 text-xs">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 bg-emerald-500 rounded-full" /> ≥90% Target
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 bg-amber-500 rounded-full" /> 75-89% Warning
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 bg-red-500 rounded-full" /> &lt;75% Critical
            </span>
          </div>
        </SectionCard>
      </div>
        ...
        <SectionCard title="Tiket Critical Aktif" className="mt-6">
          <div className="space-y-3">
            {critical.map((t) => (
              <div
                key={t.id}
                className={`flex items-center gap-4 p-3 rounded-xl border ${t.overSla ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}
              >
