    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KPICard
          ...
          subtitle="Menunggu penentuan prioritas"
          color="purple"
          icon="🎯"
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div>
          <SectionCard
            ...
            actions={
              <Button size="sm" variant="primary" onClick={() => navigate('/itlead/triage')}>
                Buka
              </Button>
            }
          >
            {triageQueue.length === 0 ? (
              ...
              <div className="space-y-2">
                {triageQueue.map((t) => (
                  <div
                    key={t.id}
                    onClick={() => navigate('/itlead/triage')}
      ...
      <div className="mt-6">
        <SectionCard
          title="Semua Tiket Aktif"
          actions={
            <Button size="sm" variant="ghost" onClick={() => navigate('/sla-monitoring')}>
              Monitoring SLA →
            </Button>
          }
        >
          <Table headers={['ID', 'Judul', 'Prioritas', 'Status', 'PIC', 'SLA', 'Aplikasi']}>
            {active.map((t) => (
              <TR key={t.id} highlight={t.overSla} onClick={() => navigate(`/user/tickets/${t.id}`)}>
                ...
                <TD>
                  <span className="text-xs text-gray-500 max-w-[120px] truncate block">{t.application}</span>
                </TD>
              </TR>
            ))}
          </Table>
