...
import { PageHeader, Button, PriorityBadge, StatusBadge, SLAIndicator, Table, TR, TD, SectionCard, KPICard } from '../../components/ui'
        ...
        {SLA_RULES.map(rule => (
          <SectionCard key={rule.priority}>
            <div className={`text-center`}>
              <div className={`w-10 h-10 rounded-full mx-auto mb-2 flex items-center justify-center text-sm font-bold ${rule.priority === 'critical' ? 'bg-red-100 text-red-700' : rule.priority === 'high' ? 'bg-orange-100 text-orange-700' : rule.priority === 'medium' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>
          ...
          {[
            ...
            <div key={pic.name} className="border border-gray-200 rounded-xl p-4">
              ...
              <div className="space-y-1.5">
                {pic.tickets.map(t => (
                  <div key={t.id} className={`flex items-center gap-2 text-xs p-2 rounded-lg ${t.overSla ? 'bg-red-50 border border-red-200' : 'bg-gray-50'}`}>
                ...
                {pic.tickets.length === 0 && <p className="text-xs text-gray-400 text-center py-2">Tidak ada tiket aktif</p>}
              </div>
            </div>
          ))}
        </div>
        ...
        <Table headers={['ID', 'Judul', 'PIC', 'Prioritas', 'Status', 'SLA', 'Aplikasi', 'Dibuat']}>
          {assigned.map(t => (
            <TR key={t.id} highlight={t.overSla} onClick={() => navigate(`/user/tickets/${t.id}`)}>
              ...
              <TD><p className="text-sm font-medium max-w-[180px] truncate">{t.title}</p></TD>
              ...
              <TD><span className="text-xs text-gray-500 max-w-[100px] truncate block">{t.application}</span></TD>
