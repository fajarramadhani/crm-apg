...
export default function ManagerApproval() {
  ...
  const handleAction = () => {
    setToast(
      action === 'approve'
        ? `Tiket ${selected?.id} disetujui. Proses deployment dapat dimulai.`
        : `Tiket ${selected?.id} ditolak. Dikembalikan ke PIC.`,
    )
    setAction(null)
    setComment('')
  ...
  return (
    <div>
      {toast && (
        <Toast message={toast} type={action === 'approve' ? 'success' : 'error'} onClose={() => setToast('')} />
      )}

      <PageHeader
        title="Manager Approval"
        subtitle="Tinjau dan berikan persetujuan untuk tiket yang memerlukan approval"
      />
      ...
      {pendingApproval.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <p className="text-sm font-semibold text-amber-800">
            {pendingApproval.length} tiket menunggu persetujuan Anda. Silakan tinjau dan berikan keputusan.
          </p>
        </div>
      )}
              ...
              pendingApproval.map((t) => (
                ...
                >
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    PIC: {t.pic} · {formatDate(t.updatedAt)}
                  </p>
                </div>
              ))
            )}
        ...
        {selected && (
          <div className="lg:col-span-2 space-y-4">
            <SectionCard title={`Review — ${selected.id}`}>
              ...
              <div className="mb-4">
                <SLAIndicator
                  slaRemaining={selected.slaRemaining}
                  overSla={selected.overSla}
      ...
      <Modal
        ...
        title={action === 'approve' ? 'Setujui Deployment' : 'Tolak Tiket'}
      >
        <div className="space-y-4">
          <div
            className={`flex items-center gap-3 p-4 rounded-xl ${action === 'approve' ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200'}`}
          >
            ...
            <div>
              ...
              <p className={`text-xs mt-0.5 ${action === 'approve' ? 'text-emerald-600' : 'text-red-600'}`}>
