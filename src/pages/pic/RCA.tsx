...
import { PageHeader, Button, SectionCard, Textarea, Input, Toast, Select } from '../../components/ui'
...
export default function RCA() {
  ...
  const [form, setForm] = useState({
    symptom: 'Sistem tidak dapat meng-generate dokumen PDF polis. Error "PDF generation failed: template not found" muncul saat klik tombol "Cetak Polis".',
    rootCause: 'File template PDF tidak ter-include dalam deployment package v2.3.1. Template file terhapus dari server production saat proses deployment automated.',
    contributing: 'Tidak ada validasi file pada proses deployment pipeline. Checklist deployment tidak mencakup verifikasi template files.',
    impact: 'Proses penerbitan polis untuk 50+ nasabah terhenti selama 2 hari. Nasabah tidak dapat menerima dokumen polis asuransi mereka.',
    solution: 'Restore file template dari backup server. Update deployment checklist untuk mencakup validasi template files. Tambahkan automated test untuk generate PDF setelah setiap deployment.',
    preventive: '1. Tambahkan validasi template file dalam CI/CD pipeline\n2. Implementasi smoke test post-deployment untuk fungsi generate PDF\n3. Monitor file integrity pada server production\n4. Buat alert jika template file hilang atau berubah',
    category: 'deployment',
    recurrence: 'tidak',
  })
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="bg-[#1E3A8A]/5 border border-[#1E3A8A]/20 rounded-xl p-4 mb-5 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-[#1E3A8A]">{ticket.id} — {ticket.title}</p>
          <p className="text-xs text-gray-500 mt-0.5">{ticket.application} · {ticket.division}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button variant="primary" onClick={handleSave}>💾 Simpan RCA</Button>
        </div>
      </div>
    </div>
  )
