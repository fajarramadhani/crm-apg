...
import { PageHeader, Button, StatusBadge, PriorityBadge, SLAIndicator, SectionCard, Textarea, Toast, Modal, ActivityTimeline, Tabs } from '../../components/ui'
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        actions={<Button variant="ghost" onClick={() => navigate('/pic/dashboard')}>← Dashboard</Button>}
      />
            ...
            <div className="flex items-start justify-between flex-wrap gap-3">
              <div>
                <div className="flex flex-wrap gap-2 mb-2">
                  ...
                  {ticket.overSla && <span className="px-2 py-0.5 bg-red-100 text-red-700 rounded-full text-xs font-bold animate-pulse">🚨 OVER SLA</span>}
                </div>
                <h2 className="text-base font-bold text-gray-900">{ticket.title}</h2>
                <p className="text-xs text-gray-500 mt-1">{ticket.application} · Requester: {ticket.requester}</p>
              </div>
              <div className="w-48">
            ...
            <div className="space-y-4">
              <SectionCard title="Update Progress">
                ...
                <div className="flex items-center justify-between mt-3">
                  <Button variant="secondary" size="sm" onClick={() => navigate('/pic/rca')}>🔍 Buka RCA Form</Button>
                  <div className="flex gap-2">
                    <Button variant="secondary" size="sm" onClick={() => setShowStatus(true)}>Ubah Status</Button>
                    <Button variant="primary" size="sm" onClick={handleUpdate} disabled={!progress}>💾 Simpan Update</Button>
                  </div>
                </div>
              </SectionCard>
              ...
              <SectionCard title="Checklist Pengerjaan">
                <div className="space-y-2">
                  {[
                    ...
                    <div key={i} className={`flex items-center gap-3 p-2.5 rounded-lg ${item.done ? 'bg-emerald-50' : 'bg-gray-50'}`}>
                      <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${item.done ? 'bg-emerald-500 text-white' : 'border-2 border-gray-300'}`}>
                        {item.done ? '✓' : ''}
                      </div>
                      <span className={`text-sm ${item.done ? 'text-gray-600 line-through' : 'text-gray-800 font-medium'}`}>{item.label}</span>
                    </div>
                  ))}
                </div>
              </SectionCard>
      ...
      <Modal open={showStatus} onClose={() => setShowStatus(false)} title="Ubah Status Tiket">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            <Button variant="secondary" onClick={() => setShowStatus(false)}>Batal</Button>
            <Button variant="primary" onClick={handleStatusChange} disabled={!nextStatus}>Ubah Status</Button>
          </div>
        </div>
      </Modal>
    </div>
