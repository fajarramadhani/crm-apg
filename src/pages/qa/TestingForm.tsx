...
const QA_TEST_CASES = [
  {
    id: 'qatc1',
    area: 'Fungsionalitas',
    title: 'Generate PDF Polis Jiwa — Happy Path',
    desc: 'Pilih nasabah baru → Generate → Verifikasi PDF',
    expected: 'PDF ter-generate dalam < 10 detik, konten benar',
  },
  {
    id: 'qatc2',
    area: 'Fungsionalitas',
    title: 'Generate PDF Polis Kesehatan & Kendaraan',
    desc: 'Uji semua tipe produk asuransi',
    expected: 'PDF tersedia untuk semua jenis polis',
  },
  {
    id: 'qatc3',
    area: 'Integrasi',
    title: 'Integrasi dengan modul Manajemen Polis',
    desc: 'Verifikasi data PDF sesuai data di database',
    expected: 'Data PDF konsisten dengan data sistem',
  },
  {
    id: 'qatc4',
    area: 'Performa',
    title: 'Load Test — Generate 20 PDF Bersamaan',
    desc: 'Simulasi 20 user generate PDF sekaligus',
    expected: 'Sistem tidak timeout, semua PDF berhasil',
  },
  {
    id: 'qatc5',
    area: 'Error Handling',
    title: 'Verifikasi Error Message',
    desc: 'Test dengan kondisi abnormal (session timeout, dll)',
    expected: 'Error message informatif, tidak crash',
  },
  {
    id: 'qatc6',
    area: 'Regression',
    title: 'Modul Manajemen Polis tidak terimbas',
    desc: 'Verifikasi fitur lain di modul tidak terdampak',
    expected: 'Semua fitur lain tetap berjalan normal',
  },
]
...
export default function TestingForm() {
  ...
  const handleSubmit = () => {
    setToast(
      recommendation === 'pass'
        ? 'Testing selesai. Tiket diteruskan ke UAT.'
        : 'Testing gagal. Tiket dikembalikan ke PIC.',
    )
  ...
  return (
    <div className="max-w-3xl">
      {toast && (
        <Toast message={toast} type={recommendation === 'pass' ? 'success' : 'error'} onClose={() => setToast('')} />
      )}
      ...
      <SectionCard className="mb-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {ticket.application} · PIC: {ticket.pic}
            </p>
          </div>
          <div className="text-right">
            <p className="text-xs text-gray-400">Progress</p>
            <p className="text-3xl font-bold text-[#1E3A8A]">
              {Object.keys(results).length}/{QA_TEST_CASES.length}
            </p>
        ...
        <div className="mt-3 h-2 bg-gray-100 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${(Object.keys(results).length / QA_TEST_CASES.length) * 100}%` }}
          />
        </div>
      </SectionCard>
          ...
          <SectionCard key={area} title={`Area: ${area}`}>
            <div className="space-y-4">
              {QA_TEST_CASES.filter((tc) => tc.area === area).map((tc, idx) => (
                <div
                  key={tc.id}
                  className={`p-4 border rounded-xl ${results[tc.id] === 'fail' ? 'border-red-200 bg-red-50/20' : results[tc.id] === 'pass' ? 'border-emerald-200 bg-emerald-50/20' : 'border-gray-200'}`}
                >
      ...
      <div className="flex justify-between items-center">
        <span className="text-sm text-gray-500">
          {allAnswered
            ...
            : `⏳ ${QA_TEST_CASES.length - Object.keys(results).length} test case belum selesai`}
        </span>
        <div className="flex gap-3">
          <Button variant="secondary" onClick={() => navigate('/qa/dashboard')}>
            Batal
          </Button>
          <Button
