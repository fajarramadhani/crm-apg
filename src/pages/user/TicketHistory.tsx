...
import { TICKETS, STATUS_LABELS, PRIORITY_LABELS, formatDate } from '../../data'
import { PageHeader, Button, StatusBadge, PriorityBadge, SLAIndicator, Table, TR, TD, FilterBar, Select, Input, OverSLABanner } from '../../components/ui'

const myTickets = TICKETS.filter(t => t.requesterId === 'u1' || t.requesterId === 'u9')
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button variant="primary" onClick={() => navigate('/user/create-ticket')}>➕ Buat Tiket</Button>}
      />

      {overSlaCount > 0 && (
        <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-4">
          <span className="text-red-500 text-xl">🚨</span>
          <p className="text-sm font-semibold text-red-700">{overSlaCount} tiket Anda melewati batas SLA dan memerlukan perhatian</p>
        </div>
      )}
      ...
      <FilterBar>
        ...
        />
        <Select
          options={[{ value: '', label: 'Semua Status' }, ...Object.entries(STATUS_LABELS).map(([v, l]) => ({ value: v, label: l }))]}
          value={statusFilter}
          onChange={e => setStatusFilter(e.target.value)}
          className="w-44"
        />
        <Select
          options={[{ value: '', label: 'Semua Prioritas' }, ...Object.entries(PRIORITY_LABELS).map(([v, l]) => ({ value: v, label: l }))]}
          value={priorityFilter}
          onChange={e => setPriorityFilter(e.target.value)}
          className="w-36"
        />
        {(search || statusFilter || priorityFilter) && (
          <button onClick={() => { setSearch(''); setStatusFilter(''); setPriorityFilter('') }} className="text-xs text-gray-500 hover:text-gray-700 underline">
            Reset
          </button>
        )}
        ...
        <Table headers={['ID Tiket', 'Judul', 'Kategori', 'Prioritas', 'Status', 'SLA', 'Dibuat', 'Diupdate']}>
          {filtered.map(ticket => (
            <TR key={ticket.id} onClick={() => navigate(`/user/tickets/${ticket.id}`)} highlight={ticket.overSla}>
              ...
              <TD><span className="text-xs capitalize text-gray-600">{ticket.category}</span></TD>
              ...
              <TD><span className="text-xs text-gray-500 whitespace-nowrap">{formatDate(ticket.createdAt)}</span></TD>
              <TD><span className="text-xs text-gray-500 whitespace-nowrap">{formatDate(ticket.updatedAt)}</span></TD>
            </TR>
          ))}
        </Table>
        {filtered.length === 0 && (
