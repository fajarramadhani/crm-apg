...
import { KPICard, Card, StatusBadge, PriorityBadge, SLAIndicator, PageHeader, Button, SectionCard } from '../../components/ui'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <SectionCard
            ...
            actions={<button onClick={() => navigate('/user/tickets')} className="text-xs text-[#1E3A8A] hover:underline">Lihat Semua →</button>}
          >
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">{ticket.application} · {formatDate(ticket.updatedAt)}</p>
                    </div>
        ...
        <div className="space-y-4">
          ...
          <SectionCard title="Aksi Cepat">
            <div className="space-y-2">
              <button onClick={() => navigate('/user/create-ticket')} className="w-full flex items-center gap-3 p-3 bg-[#1E3A8A]/5 hover:bg-[#1E3A8A]/10 rounded-xl text-sm font-medium text-[#1E3A8A] transition-colors">
                <span>➕</span> Buat Tiket Baru
              </button>
              <button onClick={() => navigate('/user/uat')} className="w-full flex items-center gap-3 p-3 bg-cyan-50 hover:bg-cyan-100 rounded-xl text-sm font-medium text-cyan-700 transition-colors">
                <span>🧪</span> UAT Menunggu Saya
              </button>
              <button onClick={() => navigate('/user/tickets')} className="w-full flex items-center gap-3 p-3 bg-gray-50 hover:bg-gray-100 rounded-xl text-sm font-medium text-gray-700 transition-colors">
            ...
            <Card className="p-4 border-cyan-200 bg-cyan-50">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-cyan-600 mt-0.5">Tiket IT-2026-000001 siap untuk UAT. Silakan lakukan pengujian.</p>
                  <button onClick={() => navigate('/user/uat')} className="mt-2 text-xs text-cyan-700 font-semibold hover:underline">
                    Mulai UAT →
                  </button>
                </div>
              </div>
